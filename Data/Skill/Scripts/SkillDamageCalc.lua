murequire("SkillDamageCalcDefine.lua")
...
function SkillDamageProc(lpObj,lpTarget,lpSkill,skill,Count,Combo,InitDamage)
	...
	CreatePlayerTable(lpObj,"lpPlayer")
	
	lpTPlayer = {}
	
	CreatePlayerTable(lpTarget,"lpTPlayer")
	
	local Class = lpPlayer.Class
	...
	local MountIndex = GetObjectMountIndex(lpObj)
		if((skill ~= SKILL_IMPALE) or MountIndex == GetItemIndex(13, 2) or MountIndex == GetItemIndex(13, 3) or MountIndex or GetItemIndex(13, 37)) then
			if(Class == CLASS_DK or Class == CLASS_SL) then
				FinalDamage = (InitDamage*GetObjectMultiplierDamage(lpObj,Class,0))/100
			else
	...
	elseif(skill == SKILL_CHAIN_DRIVER) then
		...
		FinalDamage = (FinalDamage*GetObjectMultiplierDamage(lpObj,Class,0))/100
	elseif(skill == SKILL_DARK_SIDE) then
		...
		FinalDamage = (FinalDamage*GetObjectMultiplierDamage(lpObj,Class,2))/100
	elseif(skill == SKILL_DRAGON_LORE) then
		...
		FinalDamage = (FinalDamage*GetObjectMultiplierDamage(lpObj,Class,1))/100
	elseif(skill == SKILL_DRAGON_SLAYER) then
		FinalDamage = (InitDamage*GetObjectMultiplierDamage(lpObj,Class,1))/100
		if(lpTPlayer.Type == OBJECT_USER) then
			FinalDamage = ((FinalDamage+100)*3)
		end
	elseif(skill == SKILL_CHARGE) then
		FinalDamage = (InitDamage*(150+GetObjectMultiplierDamage(lpObj,Class,0)))/100
	...
	elseif (skill == SKILL_SHININGPEAK) then
		FinalDamage = (InitDamage * ((GetObjectMultiplierDamage(lpObj,Class,0) + GetObjectMultiplierDamage(lpObj,Class,1)) * 0.8) / 100)
...
function SkillSwordPowerGetPhysiDamage(lpObj,Value,InitDamageMin, InitDamageMax)	-- SKILL_SWORD_POWER
	...
	if(Value>100--[[old SUPhysiDamageMaxConstA]]) then
		Value = 100--[[old SUPhysiDamageMaxConstA]]
	end
	
	local FinalDamageMin = InitDamageMin + (((GetObjectStatByType(lpObj,POINT_STRENGTH)+GetObjectStatByType(lpObj,POINT_DEXTERITY))/8)*Value)/100

	local FinalDamageMax = InitDamageMax + (((GetObjectStatByType(lpObj,POINT_STRENGTH)+GetObjectStatByType(lpObj,POINT_DEXTERITY))/4)*Value)/100
...
function TestTable(lpObj)
	...
	CreatePlayerTable(lpObj,"Player")

	LogAdd(LOG_RED,string.format('%d | %d |', Player.Index,Player._Index))
